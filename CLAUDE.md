...
- **Daily Token Limit**: Per-key daily token cap (units of 万/10k tokens) configured in the admin "Edit API Key" form. The usage aggregator accumulates the day's tokens (input + output + cache-write; cache read tokens are excluded; local-day boundary via `APP_TIMEZONE`); when exceeded the key is deactivated (`deactivated_reason="daily_token_exceeded"`) and auto-reactivated at the next local midnight in `validate_api_key`. Mirrors the monthly budget mechanism.
